...
class Settings(BaseSettings):
    ...
    # اگر دو پرسنلِ متفاوت (مثلاً دو خواهر/برادر) هر دو نزدیک به همین اندازه
    # به چهرهٔ روبه‌روی دوربین باشند، به‌جای حدس زدن، تشخیص رد می‌شود (کاربر
    # به کد پرسنلی/PIN هدایت می‌شود). یعنی: فاصلهٔ نفرِ دوم منهای نفرِ اول باید
    # حداقل همین‌قدر باشد وگرنه مبهم است. بیشتر = سخت‌گیرتر دربرابر افراد
    # شبیه به هم، ولی احتمال بیشترِ رد شدنِ تصادفیِ تطبیقِ درست.
    FACE_AMBIGUITY_MARGIN: float = 0.08
