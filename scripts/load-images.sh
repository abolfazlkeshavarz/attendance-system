...
#   ./scripts/load-images.sh [path/to/attendance-images.tar.gz]
#   make load-images
#
...
BUNDLE="${1:-}"
if [[ -z "$BUNDLE" ]]; then
  for candidate in attendance-images.tar.gz dist/attendance-images.tar.gz; do
    if [[ -f "$candidate" ]]; then
      BUNDLE="$candidate"
      break
    fi
...
if [[ -z "$BUNDLE" || ! -f "$BUNDLE" ]]; then
  ...
  echo "    scp dist/attendance-images.tar.gz USER@THIS_SERVER:$(pwd)/" >&2
  exit 1
fi
...
# architecture mismatch loads happily and only fails later, at container
# start, with an opaque "exec format error".
...
for img in attendance-backend:latest attendance-web:latest; do
