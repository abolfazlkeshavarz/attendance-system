...
import { GreetingCard, TopBar, useClock, type Greeting } from './parts'
import { FingerprintKiosk, fingerprintHint } from './FingerprintKiosk'
...
export default function Kiosk() {
  ...
  // این کلید به یک دربِ اثر انگشت وصل است، نه تبلت دوربین‌دار — کل جریان
  // تشخیص چهره کنار می‌رود و به‌جایش صفحهٔ اثر انگشت نشان داده می‌شود.
  const isFingerprintKiosk = kioskSettings.device_kind === 'fingerprint'
  const faceEnabled = kioskSettings.face_enabled && !isFingerprintKiosk
  const pinEnabled = kioskSettings.pin_enabled && !isFingerprintKiosk
  const { videoRef, error: cameraError, running } = useCamera(paired && !inAdminMode && faceEnabled)

  // روی تبلتِ ترکیبی (چهره + اثر انگشت) فقط برای نمایش راهنما استفاده می‌شود؛
  // روی دربِ اثر انگشت، FingerprintKiosk خودش دوباره آن را صدا می‌زند.
  const fpStatus = useFingerprintStatus(
    paired && !isFingerprintKiosk && kioskSettings.fingerprint_enabled,
  )
  ...
  return (
    <div className="relative flex h-screen w-screen flex-col overflow-hidden bg-ink-900 text-white">
      ...
      <div className="relative flex flex-1 items-center justify-center">
        ...
        {screen === 'scan' && isFingerprintKiosk && <FingerprintKiosk />}

        {screen === 'scan' && !isFingerprintKiosk && (
          <ScanOverlay
            hint={fingerprintHint(fpStatus) ?? hint}
